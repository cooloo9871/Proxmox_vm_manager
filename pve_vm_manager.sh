...
check_env() {
  ...
  if [[ ! "$?" == "0" ]]; then
    ...
    printf "${YEL}=====Run this command on $EXECUTE_NODE: apt install -y libguestfs-tools=====${NC}\n"
    exit 1
  else
...
dep_kind() {
  ...
  do
    if ! ssh -q -o "StrictHostKeyChecking no" root@"$EXECUTE_NODE" qm list | grep "$l" &>/dev/null; then
      ...
      sshpass -p "$PASSWORD" ssh -o "StrictHostKeyChecking no" "$USER"@"$VM_netid.$m" rm -rf ssh ssh.zip cookies.txt google_uuid.txt &>> /tmp/pve_vm_manager.log
